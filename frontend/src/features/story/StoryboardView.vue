...
function file(path: string | null): string { return path ? fileUrl(pid.value, path) : '' }
function thumb(card: StoryboardCard): string { return file(card.thumbnail_path) }
function fmtDuration(n: number | null | undefined): string { return n == null ? '—' : `${Math.round(n * 10) / 10}s` }
function previewShot(card: StoryboardCard): void {
  ...
  preview.value = { shotId: card.id, title: `${card.index_no}. ${card.title} · v${version.version_no}`, path: file(version.video_path) }
}
function previewVersion(card: StoryboardCard, versionId: string): void {
  const version = card.versions.find((v) => v.id === versionId)
  if (version?.video_path) preview.value = { shotId: card.id, title: `${card.index_no}. ${card.title} · v${version.version_no}`, path: file(version.video_path) }
}
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex min-h-0 flex-1 gap-2 p-2">
      ...
      <aside class="border-line-1 bg-base-1 w-80 shrink-0 overflow-auto border">
        ...
        <div v-else class="space-y-3 p-2">
          <section><p class="text-fg-3 text-2xs">第 {{ story.shot.scene_index_no }} 场 · {{ story.shot.scene_title }}</p><label class="mt-1 block"><span class="text-fg-4 text-2xs">标题</span><input :value="story.shot.title" class="border-line-1 bg-base-2 text-fg-1 mt-px h-6 w-full border px-1.5 text-2xs outline-none" @change="saveShot('title', ($event.target as HTMLInputElement).value)" /></label><div class="mt-1 grid grid-cols-2 gap-1"><label><span class="text-fg-4 text-2xs">时长</span><input :value="story.shot.duration" type="number" min="0.1" step="0.1" class="border-line-1 bg-base-2 text-fg-1 tnum mt-px h-6 w-full border px-1.5 text-2xs outline-none" @change="saveDuration(($event.target as HTMLInputElement).value)" /></label><label><span class="text-fg-4 text-2xs">状态</span><select :value="story.shot.status" class="border-line-1 bg-base-2 text-fg-1 mt-px h-6 w-full border px-1 text-2xs outline-none" @change="saveShot('status', ($event.target as HTMLSelectElement).value)"><option v-for="s in SHOT_STATUS" :key="s" :value="s">{{ SHOT_STATUS_LABEL[s] }}</option></select></label><label><span class="text-fg-4 text-2xs">机位</span><input :value="story.shot.camera ?? ''" placeholder="中景 / 特写" class="border-line-1 bg-base-2 text-fg-1 mt-px h-6 w-full border px-1.5 text-2xs outline-none" @change="saveShot('camera', ($event.target as HTMLInputElement).value)" /></label><label><span class="text-fg-4 text-2xs">运镜</span><input :value="story.shot.movement ?? ''" placeholder="推 / 摇 / 固定" class="border-line-1 bg-base-2 text-fg-1 mt-px h-6 w-full border px-1.5 text-2xs outline-none" @change="saveShot('movement', ($event.target as HTMLInputElement).value)" /></label></div></section>
          <section class="border-line-1 border-t pt-2"><p class="text-fg-3 text-2xs">角色与场景</p><p class="text-fg-2 mt-1 text-2xs">{{ story.shot.cast.length ? story.shot.cast.map((c) => c.appearance_name ?? c.character_name).join(' / ') : '没有出场角色' }}</p><p class="text-fg-4 mt-0.5 text-2xs">{{ story.shot.props.length ? story.shot.props.map((p) => p.prop_name).join(' / ') : '没有道具' }}</p><p class="text-fg-4 mt-1 text-2xs">角色、道具、地点参考和上下文账单在双击后的单 Shot 工作台中配置。</p></section>
    ...
    <div v-if="preview" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-6" @click.self="preview = null"><div class="border-line-1 bg-base-1 w-full max-w-3xl border p-2 shadow-xl"><div class="flex items-center gap-2 px-1 pb-2"><span class="text-fg-1 min-w-0 flex-1 truncate text-xs">{{ preview.title }}</span><AppButton size="sm" variant="ghost" @click="preview = null">关闭</AppButton><AppButton size="sm" variant="primary" @click="openShot(preview.shotId)">进入 Shot 工作台</AppButton></div><video :src="preview.path" controls autoplay class="bg-black max-h-[70vh] w-full" /></div></div>
  </div>
</template>
