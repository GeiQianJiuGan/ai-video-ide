...
def test_shot_split_invalid_offset(client: TestClient, pid: str) -> None:
    ...
    resp_neg = client.post(f"{API}/projects/{pid}/shots/{shot['id']}/split", json={"at_seconds": 0.05})
    assert resp_neg.status_code == 422

    resp_over = client.post(f"{API}/projects/{pid}/shots/{shot['id']}/split", json={"at_seconds": 4.0})
