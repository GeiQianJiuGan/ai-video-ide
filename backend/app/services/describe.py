...
上下文账单把它当 `desc` 冻结进版本，最后由 `providers/base.py::render_video_prompt()` 渲染成
`subject_definitions` 里那一行（`<Subject 1> is … <Picture 2>: 阿岚。褪色军绿夹克，短发，
左颊一道旧疤`）。没有它，那一行只剩一个名字——喂进去的等于只是一个文件名，
人物形象在几秒里就丢了。
...
#: 一模一样，但它说的是「这个角色的设定」而不是「这张图长什么样」。它会被当成后者带进
#: 每一个引用这张素材的镜头（`providers/base.py::render_video_prompt`），于是画面里那个人穿的衣服
