...
class Shot(Base):
    ...
    #: **这个镜头的首帧 / 末帧**（`Asset.id`，必须是图片）。
    #:
    #: 加这两列的理由只有一条：**「哪一张是首帧」必须是用户按下去的那一下，不能是猜的**。
    #: 以前镜头上没有这个字段，上下文账单只好把优先级最高的那一条（通常是角色表）
    #: 自动提拔成首帧——于是界面上给一张角色设定图标了「首帧」，模型端也真把它当第一格用，
    #: 画面从一张三视图开始。首尾帧决定「画面从哪一格开始 / 结束」，参考素材决定
    #: 「谁出场、在哪儿」，这是两件事，得有两处表达。
    #:
    #: 空表示**没有指定**：此时 `use_prev_frame` 的镜头用上游末帧（`prev_shot_id`），
    #: 否则这个镜头就是没有首帧——账单会照实说，而不是随手挑一张顶上。
    #: 刻意不加外键（与 `current_version_id` 同理，资产表反过来也引用不到镜头），
    #: 取不到时按「没有首帧」处理。
    first_frame_asset_id: Mapped[str | None] = mapped_column(String(40))
    last_frame_asset_id: Mapped[str | None] = mapped_column(String(40))
